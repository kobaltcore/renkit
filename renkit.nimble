...
task build_linux_i386, "Builds for linux (i386)":
  exec(getUrl("unknown-linux-musl"))
  exec("nimble build --styleCheck:hint -d:release --opt:size --mm:orc -d:strip --os:linux --cpu:i386 -y")
...
task build_windows_i386, "Builds for Windows (i386)":
  exec(getUrl("pc-windows-msvc"))
  exec("nimble build --styleCheck:hint -d:release --opt:size --mm:orc -d:strip -d:mingw --cpu:i386 -y")
